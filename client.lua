...
Citizen.CreateThread(function()
    ...
    elseif Locales and Locales['en'] then
        ...
        print('[zd_charinfo] Error: No localization found!')
    end
end)
...
RegisterCommand(Config.Command, function(source, args, rawCommand)
    TriggerServerEvent('zd_charinfo:getPlayerInfo')
end, false)

-- Handle player info response
RegisterNetEvent('zd_charinfo:showPlayerInfo', function(playerData)
    if not playerData then 
        print('[zd_charinfo] Error: No player data received')
        return 
    end
    ...
    if #menuOptions > 0 then
        ...
        lib.showContext('zd_charinfo_menu')
    else
        print('[zd_charinfo] Error: No info to display')
    end
end)
